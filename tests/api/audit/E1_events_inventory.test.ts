/**
 * E1 — Events inventory
 *
 * Exactly TWO events are publicly visible:
 *   ID 2  — Surbiton Exchange  (published=true, status="soon" → visible, not bookable)
 *   ID 4  — Padium launch      (published=true, status="soon" → visible, not bookable)
 *
 * IDs 3, 5 are published=false (hidden).
 * ID 1 (City Kickoff, Aug) was hard-deleted.
 * All former test event IDs are gone entirely.
 *
 * GET /api/events returns only published=true rows → 2 events.
 * Absent / unpublished IDs return 404 from GET /events/:id.
 */

import { describe, it, expect, beforeAll, afterAll } from "vitest";
import request from "supertest";
import app from "../../../artifacts/api-server/src/app.js";
import { truncateAll, seedEvent } from "../../setup/seed.js";

const ABSENT_IDS = [
  // hard-deleted test events
  "evt-000", "evt-001", "evt-002", "live-2026-07-26", "test-live",
];

beforeAll(async () => {
  await truncateAll();

  // Surbiton — publicly visible, booking not yet open
  await seedEvent({
    id: "2", title: "The Surbiton Exchange",
    date: "Thursday 10 September 2026", dateShort: "10 Sep",
    venue: "Surbiton Racquet Club", location: "Surbiton, Surrey",
    format: "Americano", price: "Free", pricePence: 0,
    status: "soon", published: true,
    eventDate: new Date("2026-09-10T17:30:00Z"), maxSpots: 16,
    description: "An evening of padel and peer exchange at one of Surrey's finest clubs. Americano format, drinks, and post-match introductions.",
  });

  // Padium launch — publicly visible, booking not yet open
  await seedEvent({
    id: "4", title: "P³ Launch — People, Padel, Places",
    date: "Thursday 15 October 2026", dateShort: "15 Oct",
    venue: "Padium", location: "Canary Wharf, London",
    format: "Americano", price: "£20", pricePence: 2000,
    status: "soon", published: true,
    eventDate: new Date("2026-10-15T17:30:00Z"),
    maxSpots: 16, courtsCount: 4,
    description:
      "An evening of curated play and new connections at Padium, Canary Wharf. " +
      "Americano format, drinks, and a room full of founders and senior professionals worth meeting.",
  });

  // Seed hidden events (published=false) — must NOT appear in public list
  await seedEvent({ id: "3", title: "The GRC Exchange",    published: false, status: "available", eventDate: new Date("2026-10-08T17:30:00Z") });
  await seedEvent({ id: "5", title: "The Year Closer",     published: false, status: "soon",      eventDate: new Date("2026-12-03T18:30:00Z") });
});

afterAll(() => truncateAll());

describe("E1 — Events inventory", () => {
  let events: Array<Record<string, unknown>> = [];

  beforeAll(async () => {
    const res = await request(app).get("/api/events");
    expect(res.status).toBe(200);
    events = res.body as Array<Record<string, unknown>>;
  });

  it("public GET /api/events returns exactly 2 events (published=true only)", () => {
    expect(events).toHaveLength(2);
  });

  it("events are the Surbiton Exchange and Padium launch — in date order", () => {
    expect(events[0]!.id).toBe("2");
    expect(events[0]!.title).toBe("The Surbiton Exchange");
    expect(events[1]!.id).toBe("4");
    expect(events[1]!.title).toBe("P³ Launch — People, Padel, Places");
  });

  it("hidden events (3, 5) do NOT appear in the public list", () => {
    const ids = events.map((e) => e.id);
    expect(ids).not.toContain("1");
    expect(ids).not.toContain("3");
    expect(ids).not.toContain("5");
  });

  it("both events have status 'soon' (not yet bookable)", () => {
    for (const e of events) expect(e.status).toBe("soon");
  });

  it("event 4 has the confirmed Padium launch fields", () => {
    const e = events[1]!;
    expect(e.venue).toBe("Padium");
    expect(e.location).toBe("Canary Wharf, London");
    expect(e.pricePence).toBe(2000);
    expect(e.maxSpots).toBe(16);
    expect(e.courtsCount).toBe(4);
  });

  it("event 4 description has no GRC/finance/debut language", () => {
    const desc = events[1]!.description as string;
    expect(desc).not.toMatch(/\bdebut\b/i);
    expect(desc).not.toMatch(/\bGRC\b/);
    expect(desc).not.toMatch(/\bfinance\b/i);
  });

  it("event 4 eventDate is 15 October 2026", () => {
    const d = new Date(events[1]!.eventDate as string);
    expect(d.getUTCFullYear()).toBe(2026);
    expect(d.getUTCMonth()).toBe(9);
    expect(d.getUTCDate()).toBe(15);
  });

  it("deleted test event IDs return 404", async () => {
    for (const id of ABSENT_IDS) {
      const res = await request(app).get(`/api/events/${id}`);
      expect(res.status, `expected 404 for ${id}`).toBe(404);
    }
  });
});
