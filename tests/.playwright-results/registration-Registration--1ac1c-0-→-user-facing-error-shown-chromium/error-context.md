...
- Name: registration.spec.ts >> Registration form >> server error (500) → user-facing error shown
- Location: web/registration.spec.ts:74:3
...
Locator: locator('[role="dialog"][aria-modal="true"]').getByText(/internal server error|something went wrong/i)
Expected: visible
Timeout: 6000ms
...
Call log:
  ...
  - waiting for locator('[role="dialog"][aria-modal="true"]').getByText(/internal server error|something went wrong/i)
...
- dialog:
  ...
  - textbox "Jane Smith": Error User
  - text: Work email *
  - textbox "jane@company.com": err@e2e.test
  - text: Company
  - textbox "Acme Corp"
  - text: Job title
